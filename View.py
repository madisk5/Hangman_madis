...
def show_message(result):
    if result == "won":
        messagebox.showinfo("Võitsid!", "Palju õnne, Sa võitsid!")
    if result == "lose":
        messagebox.showinfo("Kaotasid!", "Proovi uuesti!")
...
def draw_scoreboard(frame, data):
    if len(data) > 0:
        ...
        x = 0
        for p in data:
            dt = datetime.strptime(p.time, '%Y-%m-%d %H:%M:%S').strftime('%d.%m.%Y %H:%M:%S')
...
class View(Tk):
    ...
    def on_closing(self):
        if messagebox.askokcancel("Väljumine", "Kas oled kindel, et soovid kinni panna?"):
            self.destroy()
    ...
    def create_labels(self):
        ...
        error = Label(self.__frame_top, text='Vigased tähed', anchor='w', font=self.__default_bold, fg='blue')
